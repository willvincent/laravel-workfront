...
class WorkfrontClient
{
    ...
    {
        // Ensure the path does not begin with a slash
        if (substr($path, 0, 1) == '/') {
            $path = substr($path, 1);
        }
        ...
        $query  = $this->prepare($params, $fields, $method);

        if ($method == self::METH_POST) {
            $request = new GuzzleRequest($method, $path,
              ['form_params' => $query]);
        } else {
            $request = new GuzzleRequest($method, $path,
              ['query' => $query]);
